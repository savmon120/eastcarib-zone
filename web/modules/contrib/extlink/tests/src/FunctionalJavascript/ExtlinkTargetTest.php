...
class ExtlinkTargetTest extends ExtlinkTestBase {
  ...
  public function testExtlinkTarget(): void {
    ...
    // Link should have a target attribute.
  ...
  public function testExtlinkTargetNewWindow(): void {
    ...
    // Test changing the default title text.
    ...
    // Test when target is false it cascades to default title.
    $this->config('extlink.settings')->set('extlink_target', FALSE)->save();
    $this->config('extlink.settings')->set('extlink_target_display_default_title', TRUE)->save();
    $this->drupalGet($node->toUrl());
    $this->assertStringNotContainsString('This is a test', $link->getAttribute('title'));
  }
